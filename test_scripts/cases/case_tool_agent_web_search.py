"""
Case: Tool Agent — Web Search.

Tests KGraphToolAgent with google_web_search_tool.
"""
...
async def run() -> TestResult:
    ...
    tool_worker = KGraphToolWorker(
        ...
        llm=llm,
        system_directive="You are a web search assistant. Use the web search tool to find information for users.",
    ...
    agent = KGraphToolAgent(
        ...
        tool_manager=tm,
        tool_names=[ToolNameEnum.google_web_search_tool.value],
    )

    compiled = agent.get_compiled_graph()
    await save_png(compiled, "tool_agent_web_search_graph.png", buf)
