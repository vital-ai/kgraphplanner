#!/usr/bin/env python3
"""
Autonomous Conversation Agent Test Runner.

Dedicated runner for the autonomous conversation agent (Deep Agent + git
sandbox with persistent knowledge) test case.

Usage:
    python -m test_scripts.test_auto_conversation_agent_runner
    python -m test_scripts.test_auto_conversation_agent_runner --list
    python -m test_scripts.test_auto_conversation_agent_runner -r 1 2 3
"""
...
_log_file = os.path.join(_log_dir, f"auto_conversation_agent_runner_{_log_ts}.log")
...
CASES = [
    ("auto_conversation", "test_scripts.cases.case_auto_conversation_agent", "Auto Conversation Agent (deep agent + persistent knowledge)"),
]
...
async def run_cases(cases: list[tuple[str, str, str]], request_filter: list[int] | None = None) -> list[TestResult]:
    ...
    for idx, (key, module_path, display_name) in enumerate(cases, 1):
        ...
        if request_filter:
            print(f"  (running turn(s): {request_filter})")
        print(f"{'=' * 60}")
...
def main():
    parser = argparse.ArgumentParser(description="Autonomous Conversation Agent Test Runner")
    parser.add_argument("cases", nargs="*",
                        help="Case selectors (default: all)")
    parser.add_argument("--list", action="store_true",
                        help="List available cases and exit")
    parser.add_argument("-r", "--request", type=int, nargs="+",
                        help="Run only specific turn number(s) (1-based)")
    ...
    if args.list:
        ...
        print(f"\nConversation turns:")
        from test_scripts.cases.case_auto_conversation_agent import CONVERSATION_TURNS
        for i, tc in enumerate(CONVERSATION_TURNS, 1):
